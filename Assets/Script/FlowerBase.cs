...
public class FlowerBase : MonoBehaviour {
	...
	/// </summary>
	[SerializeField]
	private Image gauge = null;

	[SerializeField]
	private List<Sprite> gaugeSprite = null;
	...
	[SerializeField]
	private AudioClip bloom_se = null;

	[SerializeField]
	private AudioClip wether_se = null;

	[SerializeField]
	private AudioClip picked_se = null;

	[SerializeField]
	private AudioClip gainGauge_se = null;
...
//			}
			...
			var fItem = JfSceneManager.instance.fList.Find( f => f.flowerType.Equals( currentFlowerType ) );
			...
			{
				// 入っていなかったら新しく追加する
				fItem = new JfSceneManager.FlowerItem();
				fItem.flowerType = currentFlowerType;
				fItem.name = model.name;
				fItem.count = 1;
				JfSceneManager.instance.fList.Add( fItem );
